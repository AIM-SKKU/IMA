...
class TrainRunner(InferenceRunner):
    ...
    def _train(self):
        ...
        with torch.cuda.device(0):
            macs, params = get_model_complexity_info(self.model, (6, 640, 480), as_strings=True,
                                                     print_per_layer_stat=True,
                                                     verbose=True)  # model, input resolution, ..
        ...
        for idx, (image, mask) in enumerate(self.train_dataloader):
            ...
            if self.iter % self.log_interval == 0:
                self.logger.info(
                    'Train, Epoch {}, Iter {}, LR {}, Loss {:.4f}, {}, Cost {:.1f} sec, Parameter {}'.format(
            ...
            #########
            # if self.iter % 10 == 0:  # 매 10 iteration마다 업데이트
            #     writer.add_scalar('loss', reduced_loss, self.iter)
            #########

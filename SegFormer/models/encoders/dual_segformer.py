...
class RGBXTransformer(nn.Module):
    ...
    def forward_features(self, x_rgb, x_e):
        ...
        # stage 1
        x_fused, H, W = self.patch_embed1(x_fused)  # [8,19200,64], 120,160
        # B H*W/16 C
        # x_e, _, _ = self.extra_patch_embed1(x_e)
        for i, blk in enumerate(self.block1):
            x_fused = blk(x_fused, H, W)
        # for i, blk in enumerate(self.extra_block1):
            # x_e = blk(x_e, H, W)
        x_fused = self.norm1(x_fused)
        # x_e = self.extra_norm1(x_e)

        x_fused = x_fused.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()  # [8,64,120,160]
        ...
        # stage 2
        x_fused, H, W = self.patch_embed2(x_fused)
        ...
        x_fused = x_fused.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        # x_e = x_e.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()  # [8,128,60,80]
        # x_fused = (x_rgb +x_e)/2
        outs.append(x_fused)

        # stage 3
        x_fused, H, W = self.patch_embed3(x_fused)

...
class ResNet(ResNetCls):
    ...
    def forward(self, x):
        ...
        x3 = self.block3(x3)
        feats['c4'] = x3      # [2,1024,27,36]

        x4 = self.layer4(x3)  # 32
        x4 = self.block4(x4)
        feats['c5'] = x4      # [2,2048,27,36]
...
class Fusionmodel(nn.Module):
    def __init__(self, in_channels):
        ...
        self.RGB = nn.Conv2d(in_channels=self.channels*2, out_channels=self.channels,
                                kernel_size=1, stride=1, padding=0)
        self.D = nn.Conv2d(in_channels=self.channels*2, out_channels=self.channels,
                                kernel_size=1, stride=1, padding=0)
        ...
        self.W = nn.Conv2d(in_channels=self.out_channels, out_channels=self.channels,
                           kernel_size=1, stride=1, padding=0)
        self.conv1 = nn.Conv2d(in_channels=self.channels*2, out_channels=self.channels,
                                kernel_size=1, stride=1, padding=0)

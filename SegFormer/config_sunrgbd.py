...
C.dataset_path = "/home/csehong/datasets/soyun/ShapeConv/sunrgbd"	# dataset path
...
C.x_root_folder = osp.join(C.dataset_path, 'image')
C.x_format = '.jpg'
...
C.class_names = ['wall','floor','cabinet','bed','chair','sofa','table','door','window','bookshelf','picture','counter','blinds',
    'desk','shelves','curtain','dresser','pillow','mirror','floor_mat','clothes','ceiling','books','fridge',
    'tv','paper','towel','shower_curtain','box','whiteboard','person','night_stand','toilet',
...
C.pretrained_model = C.root_dir + '/pretrained/segformers/mit_b4.pth'
...
C.eval_crop_size = [530, 730] # [height weight]
...
C.log_dir = osp.abspath('log_' + C.dataset_name + '_' + C.backbone + '_only_rgb')
